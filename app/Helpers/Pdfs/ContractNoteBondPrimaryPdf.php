...
class ContractNoteBondPrimaryPdf extends TCPDF
{
    ...
    public function create(BondOrder $order, BondExecution $dealingSheet): string
    {
        ...
        // Multicell test
        $this->MultiCell(45, 5, 'Market', 0, 'L', 0, 0, '', '', true, valign: 'M');
        $this->MultiCell(90, 5, ': PRIMARY', 0, 'L', 0, 0, '', '', true, valign: 'M');
        ...
        $this->MultiCell(45, 5, 'Security Account No ', 0, 'L', 0, 0, '', '', true, true, valign: 'M');
        $this->MultiCell(135, 5, ': '.$profile->bot_security_number, 0, 'L', 0, 1, '', '', true, true, valign: 'M');
        $this->MultiCell(45, 5, 'BOT CDS No ', 0, 'L', 0, 0, '', '', true, true, valign: 'M');
        $this->MultiCell(135, 5, ': '.$order->client->bot_cds_number??"", 0, 'L', 0, 1, '', '', true, true, valign: 'M');
        if (! empty($custodian)) {
        ...
        $this->MultiCell(45, 5, 'Auction Date ', 0, 'L', 0, 0, '', '', true, valign: 'M');
        $this->MultiCell(135, 5, ': '.$dealingSheet->auction_date, 0, 'L', 0, 1, '', '', true, valign: 'M');

        $this->MultiCell(45, 5, 'Auction Number ', 0, 'L', 0, 0, '', '', true, valign: 'M');
        $this->MultiCell(135, 5, ': '.$dealingSheet->auction_number, 0, 'L', 0, 1, '', '', true, valign: 'M');
        ...
        $this->MultiCell(45, 5, 'Cost Amount', 0, 'L', 0, 0, '', '', true, valign: 'M');
    ...
    $this->Ln(5);
        ...
        $this->SetFont($this->font, '', 7, '', true);
        $this->Ln(3);
        $this->MultiCell(25.72, 4, 'Bid Code No.', 1, 'C', 0, 0, '', '', true, true, valign: 'L');
        $this->MultiCell(25.72, 4, 'Exchange Order No.', 1, 'C', 0, 0, '', '', true, true, valign: 'L');
        $this->MultiCell(25.72, 4, 'Bond Number.', 1, 'C', 0, 0, '', '', true, true, valign: 'L');
        $this->MultiCell(25.72, 4, 'Contract No.', 1, 'C', 0, 0, '', '', true, true, valign: 'L');
        $this->MultiCell(25.72, 4, 'Tendered Amount.', 1, 'C', 0, 0, '', '', true, true, valign: 'L');
        $this->MultiCell(25.72, 4, 'Allotted Amount.', 1, 'C', 0, 0, '', '', true, true, valign: 'L');
        $this->MultiCell(25.72, 4, 'Unallocated Amount.', 1, 'C', 0, 1, '', '', true, true, valign: 'L');
        ...
        $this->MultiCell(25.72, $rowheight, $dealingSheet->auction_number, 1, 'C', 0, 0, '', '', true, true, valign: 'M');
        ...
        $this->SetFont($this->font, '', 7, '', true);
        $this->Ln(3);
        $this->MultiCell(0, 0, 'We wish to advise that in accordance with your instructions, we have '.$this->typeT.' in the primary market on your account subject to the Rules and Regulations of the Bank of Tanzania. kindly transfer the total amount payable to our below account details for the settlement', 0, 'L', 0, 1, '', '', true, true, valign: 'M');
        $this->Ln(3);
        $this->MultiCell(210, 0, 'Our Settlement Instructions:', 0, 'L', 0, 1, '', '', true, true, ishtml: true, valign: 'M', fitcell: true);
        $this->Ln(3);
        $this->MultiCell(100, 0, '<b>Bank:</b>', 0, 'L', 0, 0, '', '', true, true, ishtml: true, valign: 'M', fitcell: true);
        $this->MultiCell(100, 0, '<b>NBC Limited</b>', 0, 'L', 0, 1, '', '', true, true, ishtml: true, valign: 'M', fitcell: true);

        $this->MultiCell(100, 0, '<b>Account Number:</b>', 0, 'L', 0, 0, '', '', true, true, ishtml: true, valign: 'M', fitcell: true);
        $this->MultiCell(100, 0, '<b>011103041540</b>', 0, 'L', 0, 1, '', '', true, true, ishtml: true, valign: 'M', fitcell: true);

        $this->MultiCell(100, 0, '<b>Branch:</b>', 0, 'L', 0, 0, '', '', true, true, ishtml: true, valign: 'M', fitcell: true);
        $this->MultiCell(100, 0, '<b>Corporate Branch</b>', 0, 'L', 0, 1, '', '', true, true, ishtml: true, valign: 'M', fitcell: true);

        $this->MultiCell(100, 0, '<b>Swift Code:</b>', 0, 'L', 0, 0, '', '', true, true, ishtml: true, valign: 'M', fitcell: true);
        $this->MultiCell(100, 0, '<b>NLCBTZTZ</b>', 0, 'L', 0, 1, '', '', true, true, ishtml: true, valign: 'M', fitcell: true);

        $this->Ln(3);
        $this->MultiCell(0, 0, "For any discrepancy/complaints, please contact our dealing team Email: <a href='mailto:dealing@itrust.co.tz'>dealing@itrust.co.tz</a> Telephone: +255 769 665066. If you do not notify us of any discrepancy herein within 24 hours, we will deem our details to be correct", 0, 'J', 0, 1, '', '', true, true, ishtml: true, valign: 'M', fitcell: true);

...
class Helper
{
    ...
    public static function newEventStream(): void
    {
        try{
            ...
            event(new \App\Events\DataStreamEvent($event));
...
//        $pdf->setBarcode($barcode);
    ...
    public static function pdfBarcode($pdf,$barcode): void
    {
        $style = array(
            'border' => 1,
            'vpadding' => 'auto',
            'hpadding' => 'auto',
            'fgcolor' => array(233,105,44),
            'bgcolor' => array(12,80,128),
            'module_width' => 1, // width of a single module in points
            'module_height' => 1 // height of a single module in points
        );
        $secret = base64_encode(str_ireplace("==","",$barcode));
        $pdf->write2DBarcode("itrust.co.tz", 'QRCODE,Q', 170, 273, 0, 20, $style, 'N');
...
//        $permissions = array('open','print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high');
        $permissions = array( 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble');
        $pdf->SetProtection( $permissions,  $user_pass,  $owner_pass,  $mode,  $pubkeys = null );
    }


    public static function admins(): array{
        return ['kelvin@brokerlink.co.tz','abdul@itrust.co.tz'];
    }
    ...
    public static function customerCompanyShares($id,$cid){
        $buy = \DB::table("dealing_sheets")->whereNull("deleted_at")->where("client_id",$cid)->where("status","approved")->where("security_id",$id)->where("type","buy")->sum("executed");
        $sell = \DB::table("dealing_sheets")->whereNull("deleted_at")->where("client_id",$cid)->where("status","approved")->where("security_id",$id)->where("type","sell")->sum("executed");
...
//            ->sum("volume");
    ...
    public static function mailingListTradeEvents(): array
    {
        return  MailingList::where("category","trading events")->get()->toArray();
    }

    public static function setEnv($envKey, $envValue) {
        $path = app()->environmentFilePath();
        $escaped = preg_quote('='.getenv($envKey), '/');
    ...
    public static function customerTypes(): array{
        return ['minor','individual','corporate','joint'];
    }
    ...
    public static function financialYear($business_id): FinancialYear
    {
        return FinancialYear::where("business_id",$business_id)
            ->where("status","active")
            ->first();
    }

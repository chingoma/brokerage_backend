...
class ExpensesMonthlyExport implements FromQuery, WithHeadings, WithMapping, WithStyles, WithTitle
{
    use Exportable;
    ...
    {
        $data = Transaction::query()->ExpensesReport()->groupBy('reference')->whereDate('transaction_date', '<=', $this->end)->whereDate('transaction_date', '>=', $this->from)->where('status', 'approved');

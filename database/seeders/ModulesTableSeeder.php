...
class ModulesTableSeeder extends Seeder
{
    ...
    public function run(): bool
    {
        ...
        $user = User::whereEmail('super.user@brokerlink.co.tz')->first();
        $user->assignRole('Super');

        $user = User::whereEmail('trade.inputer@brokerlink.co.tz')->first();
        $user->assignRole('Trade Inputer');

        $user = User::whereEmail('trade.auth@brokerlink.co.tz')->first();
        $user->assignRole('Trade Authorizer');

        $user = User::whereEmail('finance.inputer@brokerlink.co.tz')->first();
        $user->assignRole('Finance Inputer');

        $user = User::whereEmail('finance.auth@brokerlink.co.tz')->first();
        $user->assignRole('Finance Authorizer');

        $user = User::whereEmail('chief.officer@brokerlink.co.tz')->first();

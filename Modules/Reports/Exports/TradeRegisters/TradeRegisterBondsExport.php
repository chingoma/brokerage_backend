...
class TradeRegisterBondsExport implements FromQuery, WithHeadings, WithMapping, WithStyles, WithTitle
{
    use Exportable;
    ...
    public function map($data): array
    {
        ...
        return [
            ...
            $data->executed,
            $data->price,
            $data->brokerage / $data->executed * $data->price / 100,
            $data->brokerage_rate,
            $data->brokerage,
            $data->vat,
            $data->cmsa,
    ...
    {
        $query = BondExecution::query()->where('category', 'bond')->orderBy('trade_date', 'desc')
            ->where('uid', '!=', '')
